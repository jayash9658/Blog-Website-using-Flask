...
def home():
    ...
    last = math.ceil(total_posts / int(params['no_of_posts']))  # Use math.ceil instead of math.floor
...
def edit(sno):
    if 'user' in session and session['user'] == params['admin_user']:
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title=title,tagline=tline,content=content,img_file=img_file,date=date,slug=slug)
                db.session.add(post)
                db.session.commit()
            else:
